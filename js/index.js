...
document.addEventListener("DOMContentLoaded", () => {
	...
	createRequest(Request, (response) => {
		...
		dayLinks.forEach(dayLink => dayLink.addEventListener('click', (event) => {
			...
			//получаем значение атрибута data-spotTime целочисленного типа из элемента списка
			let spotTimeDay = Number(event.target.dataset.spotTime);
			//если spotTimeDay = NaN), то получаем spotTimeDay из ближайшего родителя элемента с классом page-nav__day
			if (isNaN(spotTimeDay)) {
				spotTimeDay = Number(event.target.closest(".page-nav__day").dataset.spotTime);
			};

			//для каждого элемента массива movieSeances
			movieSeances.forEach(movieSeance => {
				let spotTimeSeanceDay = Number(movieSeance.dataset.seanceStart) * 60;
				let spotTimeSeance = spotTimeDay + spotTimeSeanceDay;
				let spotTimeNow = Math.trunc(+new Date() / 1000);
				movieSeance.dataset.seanceTimeStamp = spotTimeSeance;

				if ((spotTimeSeance - spotTimeNow) > 0) {
